...
export class StaffRegistration extends Component {
    ...
    onSubmission=(e)=>{
        <div>{`${this.state.pancardNumber} ${this.state.fname} ${this.state.lname} ${this.state.gender} ${this.state.city} ${this.state.password} ${this.state.email} ${this.state.contact}`}</div>
        e.preventDefault()
        ...
        if(isValid){
            ...
            if(this.state){
                ...
                let staffmember={pancardNumber:this.state.pancardNumber,fname:this.state.fname,lname:this.state.lname,gender:this.state.gender,city:this.state.city,password:this.state.password,email:this.state.email,contact:this.state.contact}
                StaffMemberService.createStaffMember(staffmember);
                        ...
                        <div class="col-lg-7">
                            <div class="card2 card border-0 px-4 py-5">
                                ...
                                <div className="row px-3">
                                    ...
                                    <input value={this.state.pancardNumber} name="pancardNumber" className="row px-3" placeholder="Enter PAN No. " onChange={this.changePancardHandler}/>
                                    <div style={{color:"red"}}>{this.state.pancardError}</div>
                                </div>
                                <div className="row px-3">
                                    <label class="mb-1"><h6 class="mb-0 text-sm"> First Name </h6></label>
                                    <input value={this.state.fname} name="fname" className="row px-3" placeholder="Enter Firstname " onChange={this.changeFnameHandler} />
                                    <div style={{color:"red"}}>{this.state.fnameError}</div>
                                </div>
                                <div className="row px-3">
                                    <label class="mb-1"><h6 class="mb-0 text-sm"> Last Name </h6></label>
                                    <input value={this.state.lname} name="lname" className="row px-3" placeholder="Enter Lastname " onChange={this.changeLnameHandler}/>
                                    <div style={{color:"red"}}>{this.state.lnameError}</div>
                                </div>
                                <div className="row px-3">
                                    <label class="mb-1"><h6 class="mb-0 text-sm"> Gender </h6></label>
                                    <input value={this.state.gender} name="gender" className="row px-3" placeholder="Enter Gender " onChange={this.changeGenderHandler}/>
