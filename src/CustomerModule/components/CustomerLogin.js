...
export class CustomerLogin extends Component {
    ...
    handleSubmit = (e) =>{
        e.preventDefault();
        let that=this
        var pancardnumber=this.state.pancardNumber;
        axios.get('http://localhost:8080/customers/login/'+this.state.pancardNumber+'/'+this.state.password)
        .then(function(response){
        if(response.status === 200){
            alert(" *** LOGIN SUCCESSFUL ***")
           // alert("ENJOY THE SERVICES :)")
           axios.get('http://localhost:8080/customers/getCustomerName/'+pancardnumber)
        .then(function(resp){
           console.log(resp.data);
        
            that.props.history.push({pathname: '/customer',
            data: resp.data,
        ...
        console.log("Username does not exists");
                ...
                console.log(error);
                alert("Pancard number or Password is invalid"+error);
            });
}
                        ...
                        <div class="row d-flex">
                            ...
                            <form method="POST" onSubmit={this.handleSubmit}>
                                <div class="card2 card border-0 px-4 py-5">
                                    <label><h5> Customer Login </h5></label>
                                    ...
                                    <div class="row mb-4 px-3"> <small class="font-weight-bold">Don't have an account? <Link class="text-danger " to="/customerregister">Register</Link></small> </div>
                                </div>
                                </form>
                                <div></div>
                                <br/><br/>
